...
#save model to file
model.save("model-placement.h5")


print(final_df.head())
for i in range(categ_num):
    above4 = final_df[ (final_df["salarylevel"] == i) ]#df[ (df["salary"] > 200000) & (df["salary"] < 300000) ]#
    print(above4.head())
    above4_test = above4.drop(columns=['salarylevel'])
    results = model.predict(scaler.transform(above4_test))
    for i in range(len(results)):
        print(np.argmax(results[i]),"<==",above4.iloc[[i]]['salarylevel'].values[0])
